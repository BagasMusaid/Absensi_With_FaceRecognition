                    ...
                    <tbody id="guru-list">
                        @foreach ($datas as $item)
                            <tr
                                ...
                                <td class="px-8 py-3 text-center text-sm font-semibold capitalize">{{ $item->nama_guru }}
                                </td>
                                ...
                                <td class="px-5 py-3 text-center">
                                    <h4
                                        class="{{ trim($item->kepalasekolah) == 'ya' ? 'bg-purple-500 text-white' : 'bg-red-500 text-white' }} py-0.5 md:py-1 md:px-0.5 mx-1 md:mx-3 font-medium text-[9px] md:text-xs rounded-md">
                                        {{ $item->kepalasekolah }}
                                    </h4>
                                </td>
                                @can('akses-admin')
                                    <td class="px-5 py-3 text-center flex items-center">
                                        ...
                                        <div>
                                            ...
                                            @include('pages.guru.edit', ['item' => $item])
                                        </div>
...
@push('scripts')
    <script>
            ...
            delayTimer = setTimeout(() => {
                $.ajax({
                    ...
                    success: function(data) {
                        $("#guru-list").html($(data).find("#guru-list")
                            .html());
                        attachDeleteEvent
                        ...
                        $(document).on("click", "#edit-btn", function() {
                            ...
                            $("#" + modalId).removeClass("hidden").addClass(
                                "flex backdrop-blur-sm bg-opacity-60 drop-shadow-sm"
                            );
                        });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            let selectElement = document.getElementById("kepalasekolah");
            ...
            // Event listener untuk perubahan pilihan di dropdown
            selectElement.addEventListener("change", togglePasswordField);

            // Menjalankan fungsi saat halaman dimuat (untuk menangani old() value)
            togglePasswordField();
        });
    </script>
@endpush
