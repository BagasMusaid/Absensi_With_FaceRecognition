<!-- Edit user modal -->
<div id="edit-presensi-{{ $p->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-xl max-h-full">
        ...
        <form class="relative bg-white rounded-lg shadow dark:bg-gray-700"
            action="{{ route('presensi-siswa.update', $p->id) }}" method="POST">
            @csrf
            @method('PUT')
            <!-- Modal header -->
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button"
                    ...
                    data-modal-hide="edit-presensi-{{ $p->id }}">
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-6 w-full">
                        ...
                        <div class="relative max-w-full ">
                            ...
                            <input datepicker id="tanggal-presensi-{{ $p->id }}" name="tanggal_presensi"
                                type="text"
                                class="bg-gray-50 border {{ $errors->has('tanggal_presensi') ? 'border-red-600' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Select date"
                                value="{{ old('tanggal_presensi', \Carbon\Carbon::parse($p->tanggal ?? now())->format('m-d-Y')) }}">
                        </div>
                    ...
                    <div class="col-span-3 sm:col-span-3">
                        ...
                        <select id="status" name="status"
                            ...
                            <option value="" disabled {{ old('status', $p->status) ? '' : 'selected' }}>
                                Pilih
                                Hari Mapel
                            </option>
                            <option value="Hadir" {{ old('status', $p->status) == 'Hadir' ? 'selected' : '' }}>
                                Hadir</option>
                            <option value="Alpha" {{ old('status', $p->status) == 'Alpha' ? 'selected' : '' }}>
                                Alpha</option>
                            <option value="Sakit" {{ old('status', $p->status) == 'Sakit' ? 'selected' : '' }}>
                                Sakit</option>
                            <option value="Izin" {{ old('status', $p->status) == 'Izin' ? 'selected' : '' }}>
                                Izin</option>
                        </select>
                        @error('status')
                            <small class="mt-1 ml-1 text-[10px] text-red-600 dark:text-red-500">{{ $message }}</small>
                    ...
                    <div class="col-span-3 sm:col-span-3">
                        <label for="waktu" class="block text-gray-700 text-sm font-medium mb-2">
                            Waktu Presensi
                        </label>
                        <div class="relative">
                            ...
                            <input type="time" id="waktu" name="waktu"
                                class="bg-gray-50 border leading-none  {{ $errors->has('waktu') ? 'border-red-600' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                min="00:00" max="18:00"
                                value="{{ old('waktu', \Carbon\Carbon::parse($p->waktu_presensi)->format('H:i')) }}"
                                required />
                        </div>
                        @error('waktu')
                            <small class="mt-1 ml-1 text-[10px] text-red-600 dark:text-red-500">{{ $message }}</small>
