...
@section('content')
    <div class="p-4 sm:ml-64">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ url('laporan-siswa') }}"
                            class="ms-1 text-xs md:text-sm font-medium  {{ request()->is('laporan-siswa') ? 'text-blue-600' : 'text-gray-700' }} hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">
        ...
        <div class="p-4 bg-white dark:border-gray-700 mt-5">
            <h2 class="font-bold md:text-2xl">Cetak Laporan Data Siswa</h2>
                    ...
                    <div class="bg-gray-100 flex justify-start p-4">
                        ...
                        <div class=" ml-0 md:ml-3 px-2">
                            <select id="filter_siswa" name="filter_siswa"
                                ...
                                <option value="" disabled {{ old('filter_siswa') ? '' : 'selected' }}>Pilih Jenis
                                    Filter </option>
                                <option value="nama_siswa" {{ old('filter_siswa') == 'nama_siswa' ? 'selected' : '' }}>Nama
                                    Siswa
                                </option>
                                <option value="kelas_id" {{ old('filter_siswa') == 'kelas_id' ? 'selected' : '' }}>
                                    Kelas</option>
                            </select>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        function submitForm(actionType) {
            ...
            document.getElementById('typeAction').value = actionType;

            // Submit the form
            document.getElementById('filterFormSiswa').submit();
        }
    </script>
@endpush
