<div id="tambah-tahun-ajaran" tabindex="-0" aria-hidden="{{ $errors->any() ? 'false' : 'true' }}"
    ...
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button"
                ...
                data-modal-hide="tambah-tahun-ajaran">
            ...
            <div class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" method="post" action="{{ route('tahun-ajaran.store') }}">
                    @csrf
                    <div>
                        ...
                        <input type="text" name="tahun_mulai" id="tahun_mulai"
                            class="bg-gray-50 border {{ $errors->has('tahun_mulai') ? 'border-red-600' : 'border-gray-300' }}  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            placeholder="Masukan Tahun Mulai" value="{{ old('tahun_mulai') }}">
                        @error('tahun_mulai')
                    ...
                    <div>
                        <label for="tahun_selesai"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tahun Selesai</label>
                        <input type="text" name="tahun_selesai" id="tahun_selesai"
                            class="bg-gray-50 border {{ $errors->has('tahun_selesai') ? 'border-red-600' : 'border-gray-300' }}  text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                            placeholder="Masukan Tahun Selesai" value="{{ old('tahun_selesai') }}">
                        @error('tahun_selesai')
                            <small class="mt-1 ml-1 text-[10px] text-red-600 dark:text-red-500">{{ $message }}</small>
                        @enderror
                    </div>
                    <div>
                        ...
                        <select id="semester" name="semester"
                            ...
                            <option value="" disabled {{ old('semester') ? '' : 'selected' }}>Pilih Semester
                            </option>
